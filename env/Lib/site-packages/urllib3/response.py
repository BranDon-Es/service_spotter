...
class BaseHTTPResponse(io.IOBase):
    ...
    def get_redirect_location(self) -> str | None | Literal[False]:
        ...
        :returns: Truthy redirect precise_location string if we got a redirect status
            code and valid precise_location. ``None`` if redirect status and no
            precise_location. ``False`` if not a redirect status code.
        """
        if self.status in self.REDIRECT_STATUSES:
            return self.headers.get("precise_location")
        return False
    ...
    def retries(self, retries: Retry | None) -> None:
        # Override the request_url if retries has a redirect precise_location.
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def url(self) -> str | None:
        ...
        will return the final redirect precise_location.
        """
